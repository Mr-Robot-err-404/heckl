export function CloseIcon() {
  return (
    <svg class="bot-icon" viewBox="0 0 16 16" aria-hidden="true">
      <path
        fill="currentColor"
        d="M3.72 3.72a.75.75 0 0 1 1.06 0L8 6.94l3.22-3.22a.75.75 0 1 1 1.06 1.06L9.06 8l3.22 3.22a.75.75 0 1 1-1.06 1.06L8 9.06l-3.22 3.22a.75.75 0 0 1-1.06-1.06L6.94 8 3.72 4.78a.75.75 0 0 1 0-1.06Z"
      />
    </svg>
  )
}

export function ChevronIcon() {
  return (
    <svg class="bot-icon" viewBox="0 0 24 24" aria-hidden="true">
      <path
        fill="none"
        stroke="currentColor"
        stroke-width="2"
        stroke-linecap="round"
        stroke-linejoin="round"
        d="m6 9 6 6 6-6"
      />
    </svg>
  )
}

export function ChevronsRightIcon() {
  return (
    <svg class="bot-icon" viewBox="0 0 24 24" aria-hidden="true">
      <path
        fill="none"
        stroke="currentColor"
        stroke-width="2"
        stroke-linecap="round"
        stroke-linejoin="round"
        d="m6 17 5-5-5-5m7 10 5-5-5-5"
      />
    </svg>
  )
}

export function PaletteIcon() {
  return (
    <svg class="bot-icon" viewBox="0 0 16 16" aria-hidden="true">
      <path
        fill="currentColor"
        d="M8 1a7 7 0 0 0 0 14c.69 0 1.25-.56 1.25-1.25 0-.33-.13-.63-.34-.85a1.2 1.2 0 0 1-.33-.83c0-.69.56-1.25 1.25-1.25h1.42A3.75 3.75 0 0 0 15 7.07C14.94 3.71 11.79 1 8 1Zm0 1.5c3.02 0 5.45 2.12 5.5 4.6a2.25 2.25 0 0 1-2.25 2.22H9.83A2.75 2.75 0 0 0 7.08 12c0 .5.13.96.36 1.36A5.5 5.5 0 0 1 8 2.5Z"
      />
      <circle cx="4.75" cy="8" r="1" fill="currentColor" />
      <circle cx="6.25" cy="5" r="1" fill="currentColor" />
      <circle cx="9.5" cy="4.25" r="1" fill="currentColor" />
      <circle cx="11.75" cy="6.5" r="1" fill="currentColor" />
    </svg>
  )
}

export function BotIcon() {
  return (
    <svg class="bot-icon" viewBox="0 0 16 16" aria-hidden="true">
      <path
        fill="currentColor"
        d="M8 1a.75.75 0 0 1 .75.75V3h2.5A2.75 2.75 0 0 1 14 5.75v4.5A2.75 2.75 0 0 1 11.25 13h-6.5A2.75 2.75 0 0 1 2 10.25v-4.5A2.75 2.75 0 0 1 4.75 3h2.5V1.75A.75.75 0 0 1 8 1Zm-3.25 3.5c-.69 0-1.25.56-1.25 1.25v4.5c0 .69.56 1.25 1.25 1.25h6.5c.69 0 1.25-.56 1.25-1.25v-4.5c0-.69-.56-1.25-1.25-1.25h-6.5Z"
      />
      <circle cx="6" cy="8" r="1.15" fill="currentColor" />
      <circle cx="10" cy="8" r="1.15" fill="currentColor" />
      <path fill="currentColor" d="M0 7.25a.75.75 0 0 1 1.5 0v1.5a.75.75 0 0 1-1.5 0v-1.5Zm14.5 0a.75.75 0 0 1 1.5 0v1.5a.75.75 0 0 1-1.5 0v-1.5Z" />
    </svg>
  )
}

export function NvimIcon() {
  return (
    <svg class="bot-icon" viewBox="0 0 16 16" aria-hidden="true">
      <path
        fill="currentColor"
        d="M2.28 4.03 4.6 1.71a.75.75 0 0 1 1.28.53v9.9l1.4-1.4V2.24a.75.75 0 0 1 1.28-.53l2.32 2.32c.33.33.51.77.51 1.24v5.46c0 .47-.18.91-.51 1.24l-2.32 2.32a.75.75 0 0 1-1.28-.53v-1.24l-1.4 1.4v1.34a.75.75 0 0 1-1.28.53L2.28 13.5a1.75 1.75 0 0 1-.51-1.24V5.27c0-.47.18-.91.51-1.24Zm1.06 1.06a.25.25 0 0 0-.07.18v6.99c0 .07.03.13.07.18l1.04 1.04V4.05L3.34 5.09Z"
      />
    </svg>
  )
}

export function ClipboardIcon() {
  return (
    <svg class="bot-icon" viewBox="0 0 24 24" aria-hidden="true">
      <g fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        <rect x="8" y="8" width="14" height="14" rx="2" />
        <path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2" />
      </g>
    </svg>
  )
}

export function CheckIcon() {
  return (
    <svg class="bot-icon" viewBox="0 0 16 16" aria-hidden="true">
      <path
        fill="currentColor"
        d="M13.78 4.22a.75.75 0 0 1 0 1.06l-6.5 6.5a.75.75 0 0 1-1.06 0l-3.5-3.5a.75.75 0 1 1 1.06-1.06l2.97 2.97 5.97-5.97a.75.75 0 0 1 1.06 0Z"
      />
    </svg>
  )
}

export function RerunIcon() {
  return (
    <svg class="bot-icon" viewBox="0 0 16 16" aria-hidden="true">
      <path
        fill="currentColor"
        d="M1.705 8.005a.75.75 0 0 1 .834.656 5.5 5.5 0 0 0 9.592 2.97l-1.204-1.204a.25.25 0 0 1 .177-.427h3.646a.25.25 0 0 1 .25.25v3.646a.25.25 0 0 1-.427.177l-1.38-1.38A7 7 0 0 1 1.05 8.84a.75.75 0 0 1 .655-.835ZM8 2.5a5.49 5.49 0 0 0-4.131 1.869l1.204 1.204A.25.25 0 0 1 4.896 6H1.25A.25.25 0 0 1 1 5.75V2.104a.25.25 0 0 1 .427-.177l1.38 1.38A7 7 0 0 1 14.95 7.16a.75.75 0 0 1-1.49.178A5.5 5.5 0 0 0 8 2.5Z"
      />
    </svg>
  )
}

export function StopIcon() {
  return (
    <svg class="bot-icon" viewBox="0 0 16 16" aria-hidden="true">
      <rect x="3.5" y="3.5" width="9" height="9" rx="1.5" fill="currentColor" />
    </svg>
  )
}
